...
void display()
{
	...
	printf("\t\t\t\t\t\tSYMBOL TABLE\n");
	printf("------------------------------------------------------------------------------------------------------------------------------\n\n");
	printf("| Index  |      Name      |  Return type  | Data type |   Type     | No. of Args |             Args            |  Scope |\n");
	for(int i = 0; i < TableLength; i++) {
		Node *temp = Table[i];
		while(temp) {
			printf("|%7d | %14s | %13s | %9s | %10s | %11d | %28s|",temp->index,temp->name,temp->rtype,temp->dtype,show(temp->type), temp-> num,temp->alist);
			//printArgs(temp);
			printf("%6s  |\n", temp->scope);
			temp = temp->next;
		}
